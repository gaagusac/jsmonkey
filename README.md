...
2. ```cd jsmonkey```
3. ```npm install```
4. ```npm start samples/sample.monkey``` (check out the examples in the samples directory) or ```npm start``` to start a repl.
5. if you want to run the tests ```npm test```

Maybe I will add a playground in the future.
